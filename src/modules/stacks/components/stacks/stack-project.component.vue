          ...
          <v-col v-if="shouldShowProject(project.name)">
            ...
            <v-expansion-panel-text align="center" elevation="0">
              <v-col align="end" cols="12">
                <v-btn @click="openTechModal(project.id)" color="primary">
                  TECH RADAR
                </v-btn>
              </v-col>
              <v-row class="d-flex">
                ...
                >
                  <v-card
                    ...
                    max-width="344px"
                  >
                    <v-card-subtitle> Stack Utilizada </v-card-subtitle>
                    <v-card-title>
                      {{ item.stackName }}
                    </v-card-title>
                    <v-btn
    ...
    <v-card>
      <v-row>
        <v-col>
          ...
          <v-divider></v-divider>
          <v-card-item
            v-for="(stack) in radarHandler.stackComparisson!.techRadarResponses"
          >
            <v-card
              :color="radarHandler.resolveStackColor(stack)"
              class="pa-3 mb-3"
              variant="tonal"
            >
        ...
        <v-col>
          <adherence-percentual
            :adherence-response="radarHandler.stackComparisson!.adherenceResponse"
          >
          </adherence-percentual>
        </v-col>
      </v-row>
  ...
  export default {
    ...
    methods: {
      ...
      async openTechModal(projectId: string) {
        this.$loader.open();
        await this.radarHandler
          .getTechComparissonByIds(projectId)
